...
async def send_request(req_id, http_client, img, client_c, model_name, slice):
    try:
        ...
        post_data = {
            'client_id': client_c,
            'request_id': req_id + 1,
            'image': img, # Move tensor to CPU before sending
        ...
        response = await http_client.fetch("http://192.168.86.89:8080", method  ='POST', headers = None, body = body, request_timeout = 300)
