...
metrics_headers = ['client_id','req_id', 'model_name', 'batch_type', 'batch', 'slice', 'time_batch','total_client_time','client_exec_time', 'server_exec_time', 'total_inference_time']
...
def handle_response(data, slice, client_exec_time, server_exec_time, total_inference_time, timeP, client_c, batch_type):
    try:
        ...
        write_to_csv("results/" + client_c + '.csv', metrics_headers, [data['client_id'], data['req_id'], data['model_name'], batch_type, data['batch'], str(slice), data['time_batch'], str(client_exec_time), str(timeP), str(server_exec_time), str(total_inference_time)])
        if data['req_id'] == int(len(imgs_path) / 2):
